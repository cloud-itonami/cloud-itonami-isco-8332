...
;; ADR-2607202500: human-required gap detection + referral-draft handoff.
...
;; the proposal (`:human-required?` + `:gap`), never inferred.
...
(deftest human-required-routes-ambiguous-gap-to-public-job-board
  (let [st (fresh-store)
        env (governor/env-for-store st)
        proposal {:kind :delivery :load-id "load-1" :safety-class :low
                   :effect :propose :confidence 0.9
